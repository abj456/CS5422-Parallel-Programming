...
void input(char* infile) {
    ...
    #pragma omp parallel for schedule(dynamic, BF) collapse(2)
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
            if (i == j) {
                Dist_host[i * n + j] = 0;
...
__global__ void cal_phase3(int n, int *Dist, int B, int Round) {
    ...
    if(b_i == Round || b_j == Round) return; // skip pivot, pivot hz, pivot vt
    ...
    int hz_i = Round * BF + thread_i;
    int hz_j = b_j * BF + thread_j;
    /* idx of pivot vt block */
    int vt_i = b_i * BF + thread_i;
    int vt_j = Round * BF + thread_j;
    ...
    int tmp_data = Dist[global_i * n + global_j];
    // #pragma unroll BF
    PRAGMA_UNROLL(BF)
    for(int k = 0; k < BF; ++k) {
        tmp_data = min(tmp_data, shared_vt[thread_i][k] + shared_hz[k][thread_j]);
    }
    
    Dist[global_i * n + global_j] = tmp_data;
}
