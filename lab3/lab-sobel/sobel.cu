                ...
                for (v = -yBound; v < yBound + adjustY; ++v) {
                    for (u = -xBound; u < xBound + adjustX; ++u) {
                        ...
                        if (newX >= 0 && newX < width && newY >= 0 && newY < height) {
                            R = s[channels * (width * newY + newX) + 2];
                            G = s[channels * (width * newY + newX) + 1];
                            B = s[channels * (width * newY + newX) + 0];
...
int main(int argc, char** argv) {
    ...
    unsigned char *dev_s = nullptr, *dev_t = nullptr;
    cudaMalloc((void **)&dev_s, sizeof(unsigned char) * height * width * channels);
    cudaMalloc((void **)&dev_t, sizeof(unsigned char) * height * width * channels);

    /* Hint 2 */
    // cudaMemcpy(...) copy source image to device (filter matrix if necessary)
    cudaMemcpy(dev_s, host_s, sizeof(unsigned char) * height * width * channels, cudaMemcpyHostToDevice);
    ...
    sobel<<<gridSize, blockSize>>>(dev_s, dev_t, height, width, channels);
    cudaDeviceSynchronize();
    
    /* Hint 4 */
    // cudaMemcpy(...) copy result image to host
    cudaMemcpy(host_t, dev_t, sizeof(unsigned char) * height * width * channels, cudaMemcpyDeviceToHost);
