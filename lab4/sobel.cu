...
__global__ void sobel(unsigned char *s, unsigned char *t, unsigned height, unsigned width, unsigned channels) {
    ...
    if(rowIdx >= height) return;
    float val[Z][3] = {{0.0}};
    // printf("Hello row %d, thread %d\n", rowIdx, colIdx);

    int stride = blockDim.x;
    __shared__ unsigned char shared_mem[2 * yBound][NUM_THREADS + 2 * xBound][3];
    // printf("smem y size = %d\n", 2 * yBound);
    const int shared_mem_width = stride + 2 * xBound;

    for(int x = colIdx; x < width; x += stride) {
        for (int v = -yBound; v <= yBound; ++v) {
            for (int u = -xBound; u <= xBound; ++u) {
                int smem_y = (rowIdx + v + yBound) % (2 * yBound);
                int smem_x = (x + u + xBound) % (shared_mem_width);
                if (bound_check(x + u, 0, width) && bound_check(rowIdx + v, 0, height)) {
                    shared_mem[smem_y][smem_x][2] = s[channels * (width * (rowIdx + v) + (x + u)) + 2];
                    shared_mem[smem_y][smem_x][1] = s[channels * (width * (rowIdx + v) + (x + u)) + 1];
                    shared_mem[smem_y][smem_x][0] = s[channels * (width * (rowIdx + v) + (x + u)) + 0];
                }
                else {
                    shared_mem[smem_y][smem_x][2] = 0;
                    shared_mem[smem_y][smem_x][1] = 0;
                    shared_mem[smem_y][smem_x][0] = 0;
                }
            }
        }
        __syncthreads();

        /* Z axis of mask (vertical & horizontal) */
        #pragma unroll 2
        for(int i = 0; i < Z; ++i) {
            val[i][0] = 0.;
            val[i][1] = 0.;
            ...
            for (int v = -yBound; v <= yBound; ++v) {
                ...
                for (int u = -xBound; u <= xBound; ++u) {
                    ...
                    int smem_x = (x + u + xBound) % (shared_mem_width);
                    // if (bound_check(x + u, 0, width) && bound_check(rowIdx + v, 0, height)) {
                        const unsigned char R = shared_mem[smem_y][smem_x][2];
                        const unsigned char G = shared_mem[smem_y][smem_x][1];
                        const unsigned char B = shared_mem[smem_y][smem_x][0];
        ...
        t[channels * (width * rowIdx + x) + 2] = cR;
        t[channels * (width * rowIdx + x) + 1] = cG;
        t[channels * (width * rowIdx + x) + 0] = cB;
    }
}
...
int main(int argc, char **argv) {
    ...
    const int num_blocks = height;
    const int shared_mem_size = (num_threads + 2 * xBound) * channels * sizeof(unsigned char);

    // launch cuda kernel
    sobel<<<num_blocks, num_threads, shared_mem_size>>>(dsrc, ddst, height, width, channels);
